...
class Board:
    ...
    def clone(cls, chessboard):
        chesspieces = [[0 for x in range(Board.HEIGHT)] for y in range(Board.WIDTH)]
    ...
    @classmethod
    def new(cls):
        chess_pieces = [[0 for x in range(Board.HEIGHT)] for y in range(Board.WIDTH)]
        #pprint.pprint(chess_pieces)
        # Create pawns.
        for x in range(Board.WIDTH):
        ...
        # Create King & Queen.
        chess_pieces[2][Board.HEIGHT-1] = pieces.King(2, Board.HEIGHT-1, pieces.Piece.WHITE)
        chess_pieces[1][Board.HEIGHT-1] = pieces.Queen(1, Board.HEIGHT-1, pieces.Piece.WHITE)

        chess_pieces[2][0] = pieces.King(2, 0, pieces.Piece.BLACK)
        chess_pieces[1][0] = pieces.Queen(1, 0, pieces.Piece.BLACK)
        #pprint.pprint(chess_pieces)
    ...
    def in_bounds(self, x, y):
        ...
        #return (x >= 0 and y >= 0 and x < Board.HEIGHT and y < Board.WIDTH)
