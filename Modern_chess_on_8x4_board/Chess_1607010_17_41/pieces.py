...
class Piece():
    ...
    def remove_null_from_list(self, l):
        x = [move for move in l if move != 0 ]
        #pprint.pprint(x)
        return x
...
class Queen(Piece):
    ...
    def get_possible_moves(self, board):
        knight_moves = []
        knight_moves.append(self.get_move(board, self.x+2, self.y+1))
        knight_moves.append(self.get_move(board, self.x-1, self.y+2))
        knight_moves.append(self.get_move(board, self.x-2, self.y+1))
        knight_moves.append(self.get_move(board, self.x+1, self.y-2))
        knight_moves.append(self.get_move(board, self.x+2, self.y-1))
        knight_moves.append(self.get_move(board, self.x+1, self.y+2))
        ...
        diagonal = self.get_possible_diagonal_moves(board)
        horizontal = self.get_possible_horizontal_moves(board)
        #print((horizontal + diagonal + knight_moves))
        return horizontal + diagonal + self.remove_null_from_list(knight_moves)
